...
//画点
BMP24.prototype.drawPointRGB = function drawPointRGB(x, y, rgb) {
    if (x >= this.w || y >= this.h || x < 0 || y < 0) {
        return;
    }
    //数据第一行为图像的最底一行，颜色数据在十六进制编辑器中的排列是蓝绿红
...
/**
 * 获取像素点颜色, 返回null或{ blue:number, green:number, red:number }
 */
BMP24.prototype.getPointRGB = function getPointRGB(x, y) {
    if (x >= this.w || y >= this.h || x < 0 || y < 0) {
        return null;
    }
    //数据第一行为图像的最底一行，颜色数据在十六进制编辑器中的排列是蓝绿红
